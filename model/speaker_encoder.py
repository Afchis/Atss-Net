...
class SpeakerEncoder(nn.Module):
    def __init__(self, num_speakers):
        ...
        self.resnet18 = nn.Sequential(
            ...
            ResudialBlock(in_channels=16, kernel_size=3, padding=1),
            nn.Conv2d(in_channels=16, out_channels=32, kernel_size=3, stride=2, padding=1),
            ResudialBlock(in_channels=32, kernel_size=3, padding=1),
            nn.Conv2d(in_channels=32, out_channels=64, kernel_size=3, stride=2, padding=1),
            ResudialBlock(in_channels=64, kernel_size=3, padding=1),
            nn.Conv2d(in_channels=64, out_channels=128, kernel_size=3, stride=2, padding=1),
            ResudialBlock(in_channels=128, kernel_size=3, padding=1),
            )
    ...
    def _pool_layer(self, x):
        ...
        return torch.cat([x.mean(dim=2), F.max_pool1d(x, kernel_size=x.size(2)).reshape(x.size(0), x.size(1))], dim=1)
    ...
    def forward(self, x):
        ...
        ''' 
        out = self.resnet18(x) # shape --> [batch, 128, freq/16, time/16]
